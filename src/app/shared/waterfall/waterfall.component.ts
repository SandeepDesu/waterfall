...
export class WaterfallComponent {
  ...
  initSvg() {
    ...
    //.attr("viewBox","0 0 "+ this.screenWidth +" 380")
  ...
  initAxis() {
    ...
    // need to get the max of both sets... hard code for now
    //this.y.domain([0, d3Array.max(data, (d) => d["value"]) + 40]);
  }
  ...
  drawBars() {
    ...
    bar.append("rect")
      ...
      .attr("y", function (d) {
        let yval = d.Max();
        //console.log(["y",yval, self.y(yval)]);
        return self.y(yval);
      })
      .attr("height", function (d) {
        let dmin = d.start();
        if (dmin <= 0) dmin = self.barmin; // cutoff values that go below our axis
        let yht = Math.abs(self.y(dmin) - self.y(d.value));
        // min height
        if (yht < 2) yht = 2;
        //console.log(["yht",yht,d.start, self.y(d.end)]);
        return yht;
      })
    ...
    //     let diff = d["diff"];
    //     if(diff>=10){
    //       return self.dollarFormatter(diff);
    //     }
    //     return "";
    //   })
    //   .attr("class","text-inside")
    //   .attr("text-anchor", "middle")
    //   .attr("x", (d) => this.x(d["name"]) + 25 )
    //   .attr("y", function(d) {
    //     return Math.abs(self.y(d.start) + self.y(d.end))/2 + 5;
    //   })
    //   .attr("width", this.x.bandwidth());
  ...
  setReSizeEvent() {
    ...
    window.addEventListener('resize', function (event) {
      ...
      resizeTimer = setTimeout(function () {
        // console.log("width::::" + document.querySelector("#" + self.container).clientWidth);
        // console.log("height::::" + document.querySelector("#" + self.container).clientHeight);
        self.screenWidth = document.querySelector("#" + self.container).clientWidth;
        //                self.host.remove('svg');
